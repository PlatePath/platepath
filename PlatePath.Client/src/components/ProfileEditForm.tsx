...
import { TextField, Button, Select, MenuItem, FormControl, InputLabel, SelectChangeEvent, Alert } from '@mui/material';
...
const ProfileEditForm = () => {
  ...
  const handleChange = (
    ...
    if ('target' in e) {
      ...
      setFormData({ ...formData, [selectEvent.target.name as string]: selectEvent.target.value });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} >
      {showAlert ? (
        <Alert sx={{mb:'12px'}} severity="success">Plan submitted successfully</Alert>
      ) : null}
      <BoxContainer gap='12px' mb='12px' flexDirection='column'>
      <TextField
        label="Age"
        type="number"
        name="age"
      ...
      <FormControl fullWidth required>
        ...
        <Select name="weightGoal" value={formData.weightGoal} onChange={handleChange}>
